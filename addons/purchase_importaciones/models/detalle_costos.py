...
class ImportacionesDatos(models.Model):
    ...
    pagado = fields.Float('Pagado', help=u'Monto pagado hasta la fecha', store=True, readonly=True)
    saldo = fields.Float('Saldo', help=u'Saldo pendiente por pagar a la fecha', store=True, readonly=True)
    estado = fields.Selection([('Por pagar','Por pagar'),('Pagado','Pagado')], string="Estado", store=True, readonly=True)
