...
class invoice_discount_wizard(models.TransientModel):
    ...
    inv_type = fields.Selection([('out_invoice','Customer Invoice'),
                                 ...
                                 ('in_invoice_refund','Vendor Invoice and Refund')], string='Tipo Factura', required="1", default='out_invoice')
    ...
    def create_excel_table(self,worksheet,header_style,text_left,text_right,text_left_bold,text_right_bold,text_center,row):
        ...
        col=0
        amount_total_company_signed= amount_total_bs= amount_discount_bs= t_sub_total= t_debito_fiscal= debito_fiscal= t_discount= t_discount_bs= t_total = 0
        count = 0
        for invoice in invoice_ids:
            ...
            amount_total_bs = invoice.amount_total_company_signed
            amount_discount_bs = invoice.amount_discount * 6.96
            subtotal = amount_total_bs + amount_discount_bs
            ...
            worksheet.write(row,col+12, amount_discount_bs or 0.0, text_right)
            t_discount_bs += amount_discount_bs
            worksheet.write(row,col+13, invoice.amount_total_company_signed or 0.0, text_right)
            t_total += invoice.amount_total_company_signed
            debito_fiscal = invoice.amount_total_company_signed * 0.13
