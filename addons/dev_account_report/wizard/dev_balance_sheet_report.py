...
class dev_balance_sheet_report(models.TransientModel):
    _name = "dev.balance.sheet.report"
    ...
    @api.multi
    def print_balance_sheet_report(self):
        return self.env['report'].get_action(self,'dev_account_report.dev_balance_sheet_report')
        
    
    @api.multi
    def get_parent_account(self):
        if self.company_id.balance_sheet_account_ids:
            account_ids = self.env['account.account'].with_context({'show_parent_account':True}).search([('id','in',self.company_id.balance_sheet_account_ids.ids)],order='code')
            return account_ids
        return False
    ...
    def get_lines(self):
        ...
        if account_ids:
            for account in account_ids:
                lst = []
                count +=1
                balance = account.with_context({'date_from':self.start_date, 'date_to':self.end_date,'strict_range':True,'company_id':self.company_id.id}).balance
                com_balance = account.with_context({'date_from':self.com_start_date, 'date_to':self.com_end_date,'strict_range':True,'company_id':self.company_id.id}).balance
                lst.append({
                    'x_name':'',
